...
class FeatTrainerAuto(FeatTrainerSuper):
        ...
        for epoch in range(initial_epoch, num_epochs):
            ...
            for batch_idx, batch in enumerate(train_loader):
                ...
                if len(batch['image_batch']) > 0:
                    ...
                    self.time_tracker.stop_measuring_time(start_time_batch, "Batch before metrics",
                                                          self.print_memory_usage)

                    if batch_idx <= 4:
                        # print('Prediction shape', pred.shape)
                        self.writer.add_image('Prediction {}'.format(batch_idx), pred[0], epoch)
                        self.writer.add_image('Input {}'.format(batch_idx), batch['image_batch'][0], epoch)
            ...
            save_checkpoint_and_best(history, entry_idx=0, smaller_better=True, model=self.model,
    ...
    def evaluate(self, loader, epoch):
        ...
        #         self.calc_and_print_conf_mat(torch.cat(prediction_list), torch.cat(label_list), epoch)
    ...
    def predict_dataset(self, dataloader):
        ...
        self.time_tracker.stop_measuring_time(start_time_prediction, "Prediction whole dataset",
                                              self.print_memory_usage)
